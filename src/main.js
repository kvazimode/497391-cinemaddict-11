...
const generateFilm = (data) => {
  ...
  const film_info = {
    ...
    total_rating: getRandomInt(1, 10),
...
const filmList = generateList(generateFilm, filmMock, getRandomInt(15, 20));
...
for (let i = 0; i < 5; i++) {
  append(upcomingFilmList, cardTemplate(filmList[i]), `beforeend`);
}
